...
import { attachNumbersToPersona, detachNumbersFromPersona } from "@/lib/bland";
...
export async function POST(
  ...
  try {
    ...
    const result = await attachNumbersToPersona(id, body.numbers);
...
export async function DELETE(
  ...
  try {
    ...
    const result = await detachNumbersFromPersona(id, body.numbers);
    return Response.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return Response.json({ error: message }, { status: 500 });
