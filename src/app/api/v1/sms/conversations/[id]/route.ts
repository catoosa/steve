...
import { getSMSConversation, updateSMSConversation, deleteSMSConversation } from "@/lib/bland";
...
export async function PATCH(
  ...
  try {
    ...
    const result = await updateSMSConversation(id, body);
    return Response.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return Response.json({ error: message }, { status: 500 });
...
export async function DELETE(
  ...
  try {
    ...
    const result = await deleteSMSConversation(id);
    return Response.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return Response.json({ error: message }, { status: 500 });
