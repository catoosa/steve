...
import { resendPostCallWebhook } from "@/lib/bland";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.call_id) return Response.json({ error: "call_id is required" }, { status: 400 });

    const result = await resendPostCallWebhook(body.call_id);
