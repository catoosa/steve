...
import { createPostCallWebhook, getPostCallWebhook } from "@/lib/bland";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!callId) return Response.json({ error: "call_id query param is required" }, { status: 400 });

    const result = await getPostCallWebhook(callId);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await createPostCallWebhook(body);
    return Response.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    return Response.json({ error: message }, { status: 500 });
