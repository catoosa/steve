...
export function DeleteGuardRailButton({ id }: { id: string }) {
  ...
  async function handleDelete() {
    if (!confirm("Delete this guard rail? This cannot be undone.")) return;

    setDeleting(true);
    try {
      const res = await fetch(`/api/guard-rails/${id}`, { method: "DELETE" });
      if (!res.ok) {
      ...
      alert(e instanceof Error ? e.message : "Failed to delete guard rail");
    } finally {
      setDeleting(false);
    }
  }
